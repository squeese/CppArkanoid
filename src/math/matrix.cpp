...
const Mat4 Mat4::LookAt2(const Vec3& eye, const Vec3& target, const Vec3& up) {
  Vec3 z = (eye - target).Normalized();
  Vec3 x = Vec3::Cross(up, z).Normalized();
  Vec3 y = Vec3::Cross(z, x).Normalized();

  GLfloat dx = Vec3::Dot(x, -eye);
  GLfloat dy = Vec3::Dot(y, -eye);
  GLfloat dz = Vec3::Dot(z, -eye);

  Mat4 m;

  // m[n11] = x.x;  m[n12] = y.x;  m[n13] = z.x;  m[n14] = 0.0f;
  // m[n21] = x.y;  m[n22] = y.y;  m[n23] = z.y;  m[n24] = 0.0f;
  // m[n31] = x.z;  m[n32] = y.z;  m[n33] = z.z;  m[n34] = 0.0f;
  // m[n41] = dx;   m[n42] = dy;   m[n43] = dz;   m[n44] = 1.0f;

  // unsigned int i = 0;
  // std::cout << i << " : " << m[n11] << ", " << m[n12] << ", " << m[n13] << ", " << m[n14] << std::endl;
  // std::cout << i << " : " << m[n21] << ", " << m[n22] << ", " << m[n23] << ", " << m[n24] << std::endl;
  // std::cout << i << " : " << m[n31] << ", " << m[n32] << ", " << m[n33] << ", " << m[n34] << std::endl;
  // std::cout << i << " : " << m[n41] << ", " << m[n42] << ", " << m[n43] << ", " << m[n44] << std::endl;
  // i++;

  m[n11] = x.x;  m[n12] = y.x;  m[n13] = z.x;  m[n14] = dx;
  m[n21] = x.y;  m[n22] = y.y;  m[n23] = z.y;  m[n24] = dy;
  m[n31] = x.z;  m[n32] = y.z;  m[n33] = z.z;  m[n34] = dz;
  m[n41] = 0.0f; m[n42] = 0.0f; m[n43] = 0.0f; m[n44] = 1.0f;
